...
require('dotenv').config({ path: 'c:\\Users\\Administrator\\Desktop\\Traecodiumpr2forTrae\\Main_Project\\server\\.env' });
...
const connectDB = async () => {
  try {
    ...
    await mongoose.connect('mongodb+srv://aasif:Gandpelaat143@cluster0.khlv5.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0');
...
const createSampleGames = async () => {
  try {
    ...
    if (existingGames > 0 && !forceRecreate) {
      console.log(`${existingGames} games already exist. Skipping game creation.`);
      return;
    }

    if (forceRecreate) {
      console.log('Force flag detected. Removing existing games...');
      await Game.deleteMany({ roundId: { $regex: /^g\\d+$/ } });
      console.log('Existing sample games removed.');
    }

    const games = Array.from({ length: 10 }, (_, i) => ({
      roundId: `g${i + 1}`,
      name: `Game ${i + 1}`,
      description: `Sample game ${i + 1} description`,
      startTime: new Date(),
      endTime: new Date(Date.now() + 86400000), // 24 hours from now
      active: true,
      visible: true,
      logoUrl: `/uploads/games/game${i + 1}.png`,
      status: 'active'
    }));

    await Game.insertMany(games);
    console.log('Created 10 sample games with IDs g1-g10');
  } catch (err) {
    console.error('Error creating sample games:', err.message);
  }
};
